...
- [8. Prepare the Application in Arduino® UNO Q Device](#8-prepare-the-application-in-arduino-uno-q-device)
  ...
  - [8.3 Modify the Configuration file](#83-modify-the-configuration-file)
  - [8.4 Download the Audio files](#84-download-the-audio-files)
  - [8.5 Updating the Application Configuration](#85-updating-the-application-configuration)
...
- Window size (`1000ms`)and window stride (`500ms`) are appropriate for audio data.
- Review all settings to confirm they match your application requirements.
- Directly click the Retrain Model block in Step 2. Do not change any settings in the Create impulse, MFCC and Classifier all values at their default

![N|Solid](./images/dog_bark_detector_create_impulse.png)
...
![N|Solid](./images/dog_bark_detector_retrain_model.png)
...
![N|Solid](./images/dog_bark_detector_application_deploy.png)
...
Clone the repository and transfer the source code to the device’s application directory. The following commands should be run in the terminal or command prompt. Ensure that the device is connected via ADB before proceeding.
   ...
   git checkout
   chmod +x ./Others/IoT-Robotics/Dog_Bark_Detector/setup_dog_bark_detector.sh
   ./Others/IoT-Robotics/Dog_Bark_Detector/setup_dog_bark_detector.sh
   ```

![N|Solid](./images/dog_bark_detector_source_setup.png)
...
Use the model file downloaded from Edge Impulse in the previous step (Section 7: Build and Download Deployable Model). Transfer this `.eim` file to the device using `adb push` or your preferred file transfer method.

For reference on building and downloading the model, see:
[Build and Deploy Model](../../../Others/IoT-Robotics/Dog_Bark_Detector/README.md#7-build-and-download-deployable-model)
...
### 8.5 Updating the Application Configuration

- After uploading the audio files, update the application configuration to reference the correct file names.

- Open the following file: `/home/arduino/ArduinoApps/Dog_Bark_Detector/assets/app.js`

- Locate the `loadSampleAudios()` function and ensure that the localAudioFiles array contains the following entries:
...
```
__Note__: These file names must exactly match the audio files placed in the assets/audio directory. The application uses these names to load and present the sample audio files through the user interface.
...
- Once your application is configured and built in Arduino App Lab, it can be deployed and executed directly on the Arduino UNO Q. This section will guide you through launching the application.
 ...
 ![N|Solid](./images/dog_bark_detector_application_view.png)

- Run the application by clicking the **Run** button in the top right corner of the Arduino App Lab window. 

 ![N|Solid](./images/dog_bark_detector_application_run.png)
...
Load the audio files and click **Run Classification**. The application displays the detected class name along with the corresponding confidence value.
 ...
 ![N|Solid](./images/dog_bark_detector_output.png)
